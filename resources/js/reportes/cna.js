...
(() => {
  ...
  async function loadData(url = null) {
    ...
    if (tabla) tabla.innerHTML = `<div class="cna-skeleton">Cargando…</div>`;
  ...
  function hookPagination() {
    // soporta paginación tailwind (nav a[href])
    document.querySelectorAll('#tablaCna nav a[href]').forEach(a => {
