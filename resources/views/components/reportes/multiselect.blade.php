...
<div class="space-y-1">
    ...
    <div class="relative" data-dropdown data-multiselect="{{ $name }}" data-title="{{ $title }}" data-empty="{{ $empty }}">
        <button
            type="button"
            class="flex w-full items-center justify-between gap-2 rounded-md border border-kp-border bg-white px-3 py-2 text-left text-sm font-semibold text-kp-ink shadow-sm kp-focus hover:bg-slate-50"
            data-dropdown-button
            data-ms-button>
            <span>{{ $title }}: {{ $empty }}</span>
            <span class="text-kp-muted" aria-hidden="true">v</span>
        </button>
        ...
        <div class="absolute left-0 right-0 z-40 mt-2 hidden rounded-md border border-kp-border bg-white p-2 shadow-lg" data-dropdown-panel>
            ...
            <div class="mt-2 flex items-center justify-between gap-2 border-t border-kp-border pt-2">
                <button type="button" class="rounded-md px-3 py-1.5 text-xs font-semibold text-kp-muted hover:bg-slate-100" data-ms-clear>
                    Limpiar
                </button>
                <button type="button" class="rounded-md bg-kp-green px-3 py-1.5 text-xs font-semibold text-white hover:bg-kp-green-dark" data-ms-apply>
                    Aplicar
                </button>
            </div>
        </div>
